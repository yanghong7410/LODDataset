...
normal_input_folder = r'./test/normal'
dark_input_folder = r'./test/dark'

# normal和dark RGB保存路径
normal_output_folder = r'./test/normal2'
dark_output_folder = r'./test/dark2'
...
for filename in sorted(os.listdir(normal_input_folder)):
    if filename[-3:] == 'CR2': # 判断文件是否是RAW格式
        ...
        if osp.exists(osp.join(dark_input_folder, dark_name)): # 若存在对应dark图片
            ...
            dark_img = raw2rgb(dark_img4c, normal_raw).transpose((1, 2, 0))
            dark_img = (dark_img * 255).round().astype(np.uint8)
